...
public class CobaltBlastFurnaceBlock extends BlockWithEntity {

    public static final DirectionProperty FACING = Properties.HORIZONTAL_FACING;

    public static final BooleanProperty LIT = Properties.LIT;

    public CobaltBlastFurnaceBlock(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(
        ...
        if (!(be instanceof CobaltBlastFurnaceBlockEntity controller)) {
            return ActionResult.PASS;
        }

        if (!controller.isMultiblockValid()) {
            player.sendMessage(
                    Text.translatable("message.wpcworld.cobalt.invalid_structure"),
                    true
            );
            return ActionResult.CONSUME;
        }

        player.openHandledScreen((CobaltBlastFurnaceBlockEntity) world.getBlockEntity(pos));
        return ActionResult.CONSUME;
    }

    @Override
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new CobaltBlastFurnaceBlockEntity(pos, state);
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return world.isClient ? null : checkType(type, ModBlockEntities.COBALT_BLAST_FURNACE_BE, CobaltBlastFurnaceBlockEntity::tick);
    }
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos,
        ...
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof CobaltBlastFurnaceBlockEntity be) {
