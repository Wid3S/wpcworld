...
public class ExcavatorUsageEvent implements PlayerBlockBreakEvents.Before {
    ...
    public boolean beforeBlockBreak(World world, PlayerEntity player, BlockPos pos,
        ...
        if (!(mainHandItem.getItem() instanceof ExcavatorItem)) {
            return true;
        }
        ...
        if (!mainHandItem.getOrCreateNbt().getBoolean(ExcavatorItem.MODE_KEY)) {
            return true;
        }
        ...
        for (BlockPos target : ExcavatorItem.getBlocksToBeDestroyed(1, pos, serverPlayer)) {
